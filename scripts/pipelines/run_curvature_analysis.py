...
def run_curvature_analysis_groups(
    *,
    ...
    """Run curvature analysis from explicit groups and return outputs.
    ...
    all_curvatures: list[list[float]] = []
    summary_rows: list[dict[str, object]] = []
    for label, record_ids in zip(labels, id_lists):
        curvatures = summarize_curvature_from_ids(
            record_ids,
            bodypart=bodypart,
            how=how,
        ...
        summary_rows.extend(
            ...
            for record_id, curvature in zip(record_ids, curvatures)
        )

    summary_df = pd.DataFrame(summary_rows)

    excel_path = curvature_analysis_dir / f"{out_name}_{how}_{bodypart.lower()}_sw_{smoothing_window}_lt_{likelihood_threshold}_st_{speed_thresh}_curvature_summary.xlsx"
    summary_df.to_excel(excel_path, index=False)

    ax = plot_group_comparison(
        *all_curvatures,
        labels=labels,
        ylabel=f"{how.capitalize()} curvature",
        test=test,
        plot_type=plot_type,
    )

    ax.set_title(f"{out_name}: {bodypart} curvature")
    plt.tight_layout()

    fig_path = curvature_analysis_dir / f"{out_name}_{how}_{bodypart.lower()}_sw_{smoothing_window}_lt_{likelihood_threshold}_st_{speed_thresh}_curvature_{plot_type}plot.pdf"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run curvature analysis from explicit ID groups and labels."
    )
